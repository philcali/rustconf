...
impl CodeGenerator {
    ...
    fn generate_module_content(&self, module: &YangModule) -> Result<String, GeneratorError> {
        ...
        for typedef in &module.typedefs {
            content.push_str(&self.generate_typedef(typedef)?);
            content.push('\n');
        }
    ...
    fn generate_typedef(&self, typedef: &crate::parser::TypeDef) -> Result<String, GeneratorError> {
        use crate::parser::TypeSpec;

        let mut output = String::new();

        // Generate rustdoc comment from YANG description
        if let Some(ref description) = typedef.description {
        ...
        if needs_validation {
            ...
            let validated_type_name = self.get_validated_type_name(&typedef.type_spec);
            output.push_str(&format!(
                "pub type {} = {};\n",
                type_name, validated_type_name
            ));
        } else {
            // Generate a simple type alias
            let base_type = self.generate_leaf_type(&typedef.type_spec, true);
    ...
    fn collect_validated_types(
        ...
        for typedef in &module.typedefs {
            if self.needs_validation(&typedef.type_spec) {
                let type_name = self.get_validated_type_name(&typedef.type_spec);
